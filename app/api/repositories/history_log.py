"""SQL-backed persistence adapter for decision history."""
...
class HistoryLogRepository:
    ...
    def save_items(self, *, owner_user_id: str, items: list[dict[str, Any]]) -> None:
        with connection_scope(self._database_url) as connection:
            ...
            connection.execute(
                text("DELETE FROM history_log WHERE owner_user_id = :owner_user_id"),
                {"owner_user_id": owner_user_id},
            )
            connection.execute(
                text(
                ...
                INSERT INTO history_log (
                    ...
                    :owner_user_id, :id, :action, :title, :description,
                    :candidate, :cuisine, :applied_cuisine_ids,
                    :matched_taste_tag_ids, :applied_scene_tag_ids,
                    :applied_budget_id, :applied_dining_mode_ids, :created_at
                )
                """
    ...
    def _ensure_table(self, connection) -> None:
        connection.execute(
            ...
            CREATE TABLE IF NOT EXISTS history_log (
                ...
                candidate JSON NULL,
                cuisine JSON NULL,
                applied_cuisine_ids JSON NOT NULL,
                matched_taste_tag_ids JSON NOT NULL,
                applied_scene_tag_ids JSON NOT NULL,
                applied_budget_id VARCHAR(64),
                applied_dining_mode_ids JSON NOT NULL,
        ...
        create_index_if_missing(
            connection,
            table_name="history_log",
            index_name="idx_history_owner_user_id",
            create_sql=(
                "CREATE INDEX idx_history_owner_user_id "
                "ON history_log (owner_user_id)"
            ),
        )
        count = connection.execute(
            text("SELECT COUNT(*) FROM history_log")
        ).scalar_one()
    ...
    def _serialize_item(
        ...
        return {
            ...
            "candidate": json.dumps(item.get("candidate"), ensure_ascii=False)
            if item.get("candidate") is not None
            else None,
            "cuisine": json.dumps(item.get("cuisine"), ensure_ascii=False)
            if item.get("cuisine") is not None
            else None,
            "applied_cuisine_ids": json.dumps(
                item.get("applied_cuisine_ids", []), ensure_ascii=False
            ),
            "matched_taste_tag_ids": json.dumps(
                item.get("matched_taste_tag_ids", []), ensure_ascii=False
            ),
            "applied_scene_tag_ids": json.dumps(
                item.get("applied_scene_tag_ids", []), ensure_ascii=False
            ),
            "applied_budget_id": str(item["applied_budget_id"])
            if item.get("applied_budget_id") is not None
            else None,
            "applied_dining_mode_ids": json.dumps(
                item.get("applied_dining_mode_ids", []), ensure_ascii=False
            ),
